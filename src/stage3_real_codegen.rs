...
fn write_and_compile_rust_safe(
    rust_code: &str,
    output_name: &str,
    ...
    let program_output = if run_result.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&run_result.stderr);
        return Err(CodeGenError::Execution(format!(
            "Program failed: {}",
            stderr
        )));
    };
...
fn validate_generated_code(rust_code: &str) -> CodeGenResult<()> {
    ...
    for (line_no, line) in lines.iter().enumerate() {
        ...
        if line.trim().ends_with("{")
            ...
            && !line.contains("for ")
        {
            return Err(CodeGenError::Syntax(format!(
                "Line {}: Suspicious opening brace: {}",
                line_no + 1,
                line.trim()
            )));
        }
    ...
    if !rust_code.contains("fn main") && rust_code.contains("println!") {
        return Err(CodeGenError::Syntax(
            "Generated code appears to need a main function".to_string(),
        ));
    }

    // Check for likely type errors
    if rust_code.contains("Vec<>") {
        return Err(CodeGenError::TypeCheck(
            "Empty Vec<> type parameter".to_string(),
        ));
    }
